 ...
 * Версия: 3.0 ULTRA
 * Файл: server/controllers/referralController.js
 * Описание: Контроллер реферальной системы (ссылки, статистика, бонусы)
 */
...
const REFERRAL_CONFIG = {
    registrationBonus: 50,   // бонус за регистрацию по реферальной ссылке
    saleBonus: 100,          // бонус за первую продажу приглашённого
    maxReferrals: 100,       // максимальное количество приглашённых для отображения
    bonusDelayDays: 7        // через сколько дней после регистрации начисляется бонус за продажу
};
...
async function updateReferralStats(userId) {
    ...
    await set(`referral:stats:${userId}`, stats.rows[0], CACHE_TTL.stats);
    return stats.rows[0];
}
...
async function getReferralInfo(req, res) {
    try {
        ...
        await set(cacheKey, response, CACHE_TTL.info);
...
async function getTopReferrers(req, res) {
    ...
    try {
        ...
        await set(cacheKey, topReferrers, CACHE_TTL.stats);
...
async function regenerateReferralCode(req, res) {
    try {
...
async function sendInvitation(req, res) {
    ...
    try {
        ...
        // Сохраняем историю отправленных приглашений
...
async function getReferralSystemStats(req, res) {
